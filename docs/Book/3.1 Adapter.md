...
- **Bridge**: Mẫu Bridge tách rời một lớp ra khỏi việc triển khai của nó, giúp cho việc triển khai có thể thay đổi độc lập mà không ảnh hưởng đến mã của lớp. Mục tiêu của Bridge là tách biệt và linh hoạt giữa abstractions và implementations.
...
- **Proxy**: Cung cấp một đại diện cho một đối tượng khác, kiểm soát việc truy cập đến đối tượng gốc. Proxy giữ nguyên interface và kiểm soát việc truy cập đối tượng thực.
...
Adapter Pattern hữu ích khi bạn cần tích hợp một thư viện hoặc một hệ thống cũ mà không thể (hoặc không nên) thay đổi. Tuy nhiên, bạn không nên sử dụng nó chỉ vì muốn giữ lại tất cả mã nguồn cũ mà không có lý do chính đáng.

### 2. **Hiệu năng có thể bị ảnh hưởng**

Thêm một lớp trung gian giữa hai hệ thống có thể làm giảm hiệu năng. Dù hiệu suất không phải lúc nào cũng là vấn đề, nhưng trong một số trường hợp đặc biệt, việc này có thể tạo ra một chi phí không mong muốn.
