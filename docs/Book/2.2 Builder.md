...
Định nghĩa Builder : Sau đó, chúng ta định nghĩa lớp Builder với các phương thức xây dựng, nhưng với Fluent Builder, mỗi phương thức xây dựng trả về chính builder, cho phép chúng ta gọi tiếp theo một cách liền mạch:
...
```
Bây giờ, chúng ta có th sử dụng Builder để xây dựng sản phẩm một cách dễ đọc và gần gũi:

```java
public class Main {
    public static void main(String[] args) {
        // Sử dụng Fluent Builder để xây dựng sản phẩm
...
- Khi sử dụng Fluent Builder, các phương thức xây dựng có thể được gọi nối tiếp trên một đối tượng builder duy nhất, giúp giảm bớt sự phức tạp trong mã nguồn và làm cho mã trở nên rõ ràng hơn.
